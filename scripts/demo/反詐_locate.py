...
使用方式：
    ...
    # 定位 + 掃描當前頁面
    regions = locate_line_regions(monitor=None)
    ...
    # 切換頁面（自動重新掃描）
    regions = switch_page(regions, "chat", monitor=None)
...
def locate_line_regions(monitor=None):
    ...
    # 演示版加寬到 r=540（原版 r=412 太窄，無法抓到「仁輝JAMES」這種長名）
    # 右邊預留空間給「搜尋/通話/視訊/選單」按鈕（從 x≈560 開始）
    chat_title_raw = {"l": 366, "t": 63, "r": 540, "b": 94}
...
def switch_page(regions, target_page, monitor=None):
    """
...
def find_unread_badges(monitor=None):
    """
...
def share_contact_card(regions, share_who, share_to, monitor=None):
    """
...
# ============================================================
def screenshot_chat_area(regions, monitor=None):
    """
    截取 LINE 對話區圖片。
...
# ============================================================
def search_friend_and_scan(regions, friend_name, monitor=None):
    """
...
def enter_chat_from_search(friend_pos, regions, monitor=None):
    """
